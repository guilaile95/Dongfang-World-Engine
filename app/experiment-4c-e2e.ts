...
async function main(): Promise<void> {
  ...
  const unrelatedBefore = hybrid
    ? authorityObserverSlice(session.store, worldId, hybrid.id)
    : { memories: [], knownClaims: [] };
  const started = Date.now();

  process.stderr.write(
    `experiment-4c-e2e npc=${roommate.id} model=${publicConfig.model} world=${publicConfig.worldFile}\n`,
  );
  ...
  try {
    ...
    let unrelatedAfter = unrelatedBefore;
    if (parsed) {
      ...
      if (hybrid) {
        unrelatedAfter = authorityObserverSlice(session.store, worldId, hybrid.id);
      }
    }
    ...
    const unrelatedUnchanged = JSON.stringify(unrelatedBefore) === JSON.stringify(unrelatedAfter);
    const wroteNpcMemory = afterWrite.memories.some(
      (row) => row.characterId === roommate.id && !beforeState.memories.some((old) => old.id === row.id),
    );
    ...
    const receipt = {
      protocol: "experiment-4c-e2e-addressee-memory",
      follows: "experiment-4b-e2e-proposal-schema-contract",
      uniqueVariable: "bind spoken memory_note without characterId to resolved addressee; fail-closed parse path not under test",
      ...
      checks: {
        ...
        unrelatedNpcUnchanged: unrelatedUnchanged,
        wroteNpcMemory,
      },
      structuralBlocker,
    ...
    assertNoSecret(JSON.stringify(receipt), config.apiKey, "experiment-4c receipt");
    const out = resolve("data/local/experiment-4c-e2e-receipt.json");
    writeFileSync(out, `${JSON.stringify(receipt, null, 2)}\n`);
    process.stderr.write(
      `4c parsed=${parsed} submitted=${t1.interpretation.submitted} npcAuth=${npcAuthorityChanged} wroteNpc=${wroteNpcMemory} unrelatedOk=${unrelatedUnchanged} passed=${passed}\n`,
    );
    if (interpCall) {
